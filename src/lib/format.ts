...
// Ported verbatim: enriches a scored row with display strings + sparkline history.
export function adaptRow(r: any): AdaptedRow {
  ...
  r.ind = r.industry || '—';
  r.g = [r.subA, r.subB, r.subC, r.subD, r.subE];
  r.priceStr = isFinite(r.price) ? '$' + r.price.toFixed(2) : '—';
  r.capStr = fmtCap(r.marketCap);
  r.hiStr = isFinite(r.yearHigh) ? '$' + r.yearHigh.toFixed(2) : '—';
  r.loStr = isFinite(r.yearLow) ? '$' + r.yearLow.toFixed(2) : '—';
  r.betaStr = isFinite(r.beta) ? r.beta.toFixed(2) : '—';
  r.volStr = isFinite(r.volume) ? (r.volume / 1e6).toFixed(1) + 'M' : '—';
  r.roicStr = isFinite(r.roic) ? (r.roic * 100).toFixed(1) + '%' : '—';
  r.evStr = isFinite(r.evEbitda) ? r.evEbitda.toFixed(1) : '—';
  r.pfcfStr = isFinite(r.pfcf) ? r.pfcf.toFixed(1) : '—';
  r.peStr = isFinite(r.trailingPE) ? r.trailingPE.toFixed(1) : '—';
  const base = r.composite;
  const hist: number[] = [];
  for (let i = 7; i >= 0; i--) hist.push(clampN(Math.round(base - i * 1.5 + Math.sin(i) * 2), 1, 100));
  r.hist = hist;
...
export function parseNews(j: any, sym: string): { items: any[]; avg: number } {
  ...
  const items = feed
    .map((a: any) => {
      ...
      const mine = ts.find((x: any) => String(x.ticker).toUpperCase() === sym.toUpperCase());
      if (mine) {
    ...
    .sort((x: any, y: any) => (isFinite(y.relevance) ? y.relevance : 0) - (isFinite(x.relevance) ? x.relevance : 0));
  const avg = items.length
    ? items.reduce((s: number, i: any) => s + (isFinite(i.score) ? i.score : 0), 0) / items.length
    : NaN;
  return { items, avg };
}

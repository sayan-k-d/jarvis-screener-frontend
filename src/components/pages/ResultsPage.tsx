...
export default function ResultsPage({ onOpenStock }: { onOpenStock: (t: string) => void }) {
  const { rows, meta, liveOn, loading, error, progress, runLive, hasKey, loadLast } = useScreener();
  ...
  const filtered = useMemo(() => {
    ...
    let out = rows.filter(
      (s) =>
        (!ql || s.t.toLowerCase().includes(ql) || (s.n || '').toLowerCase().includes(ql)) &&
        (!tier || s.tier === tier) &&
        (!sector || s.s === sector) &&
        s.sc >= minScore &&
  ...
  const tierCounts = useMemo(() => {
    const c: Record<string, number> = { All: rows.length, 'High Alert': 0, Watch: 0, 'Breakdown Watch': 0, Neutral: 0 };
    rows.forEach((s) => {
  ...
  const reset = () => {
    setQ(''); setTier(''); setSector(''); setMinScore(0); setSort('rank'); setTierPill('All'); setPage(1);
  };

  const exportCsv = () => {
    const head = ['Rank', 'Ticker', 'Company', 'Sector', 'Score', 'Delta', 'Tier', 'Confluence'];
    const lines = [head.join(',')].concat(
      filtered.map((s) =>
        [s.r, s.t, `"${(s.n || '').replace(/"/g, '""')}"`, s.s, s.sc, Math.round(s.d || 0), s.tier, s.conf ? 'Yes' : 'No'].join(','),
      ),
    );
  ...
  const win: number[] = [];
  for (let i = Math.max(1, curPage - 2); i <= Math.min(pages, Math.max(5, curPage + 2)) && win.length < 5; i++) win.push(i);
  ...
  return (
    <div className="card">
      ...
      {error && (
        <div style={{ padding: '12px 18px', color: 'var(--red)', fontSize: 12.5, borderBottom: '1px solid var(--line)' }}>
          {error}
        </div>
      )}
      ...
      <div className="toolbar">
        <input className="inp" placeholder="Search by ticker or company" value={q} onChange={(e) => { setQ(e.target.value); setPage(1); }} />
        <select className="sel" value={tier} onChange={(e) => { setTier(e.target.value); setPage(1); }}>
          <option value="">All Tiers</option>
          {['High Alert', 'Watch', 'Neutral', 'Weakening', 'Deteriorating'].map((t) => (
            <option key={t}>{t}</option>
          ))}
        </select>
        <select className="sel" value={sector} onChange={(e) => { setSector(e.target.value); setPage(1); }}>
          <option value="">All Sectors</option>
          {sectors.map((s) => (
            <option key={s}>{s}</option>
          ))}
        </select>
        <select className="sel" value={minScore} onChange={(e) => { setMinScore(+e.target.value); setPage(1); }}>
          {[0, 30, 50, 70, 85].map((v) => (
      ...
      {liveOn && (
        <div className="pills">
          {Object.entries(tierCounts).map(([k, v]) => (
            <button key={k} className={`pill ${tierPill === k ? 'active' : ''}`} onClick={() => { setTierPill(k); setPage(1); }}>
              {k} ({v.toLocaleString()})
            </button>
          ))}
        </div>
      ...
      <table>
        <thead>
          <tr>
            ...
            <th>Δ vs Prior</th>
            <th>Tier</th>
            <th>Triple Confluence</th>
          </tr>
        </thead>
        <tbody>
          {loading && !slice.length ? (
            <tr>
              <td colSpan={8} style={{ padding: 48, textAlign: 'center', color: 'var(--muted)' }}>
                <span className="spin" />
                <div style={{ marginTop: 12, fontSize: 12.5 }}>Loading live screener data…</div>
              </td>
            </tr>
          ) : slice.length ? (
            slice.map((s) => (
              <tr key={s.t}>
                ...
                <td>{s.conf ? <span className="star">★</span> : <span style={{ color: 'var(--muted)' }}>—</span>}</td>
              </tr>
            ))
          ) : (
            <tr>
              <td colSpan={8} style={{ padding: 34, textAlign: 'center', color: 'var(--muted)' }}>
                {rows.length
      ...
      {!!slice.length && (
        <div className="pager">
          <div className="pgs">
            ...
            {win.map((i) => (
              <button key={i} className={`pg ${i === curPage ? 'active' : ''}`} onClick={() => setPage(i)}>
                {i}
              </button>
            ))}
          ...
          <div>
            Showing {filtered.length ? (curPage - 1) * PER + 1 : 0} to {(curPage - 1) * PER + slice.length} of{' '}
